...
import * as redbookConnector from './redbookConnector.js';
...
async function forwardToRedbook(kind, payload, options = {}) {
  try {
    const taskOptions = {
      ...options,
      // One native Beav queue item represents one Redbook collector task. Keep
      // that identity across every note emitted by a homepage/baseline run so
      // latest-task filtering remains meaningful after ingestion.
      taskId: options.taskId || xhsActiveTask?.id || null,
      capturedAt: options.capturedAt || xhsActiveTask?.startedAt || null,
    };
...
async function handleMessage(message, sender) {
  ...
  switch (message?.type) {
    case 'page-state:update':
      ...
      return { success: true };
    case 'healthcheck':
      return await checkRedbookAwareHealth(message?.forceRefresh === true);
...
function extractXhsBloggerPayload() {
  ...
  const stateUser = unwrapValue(initialState?.user?.userPageData)
    ...
    || {};
  const stateBasic = unwrapValue(stateUser.basic_info || stateUser.basicInfo) || stateUser;
  const profileRoot =

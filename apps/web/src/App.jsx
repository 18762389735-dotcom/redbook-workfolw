...
const copy = { home: ['首页', '管理每日采集与内容运营节奏。'], knowledge: ['知识库', '本批次不实现知识库业务。'], publishing: ['发布计划', '本批次不实现发布或自动发布。'], review: ['数据复盘', '本批次不实现数据复盘。'], settings: ['设置', '本地 Collector 的使用说明见 README。'] };
export function App() {
  const [activePage, setActivePage] = useState('home'); const [profile, setProfile] = useState(null);
  useEffect(() => { getAccount().then(setProfile).catch(() => {}); }, []);
  let content;
  if (activePage === 'discover') content = <DiscoverPage />;

...
import { createWritingDraft, getWritingWorkspace, updateWritingDraft } from '../api';
...
export function WritingSelectionPage({ onNavigate }) {
  ...
  return <>
    ...
    {!draft ? <BriefCard brief={brief} opportunity={opportunity} busy={busy} onGenerate={generate} onBack={() => onNavigate('opportunities')} /> : <DraftEditor brief={brief} title={title} body={body} busy={busy} onTitle={setTitle} onBody={setBody} onSave={save} onBack={() => onNavigate('opportunities')} />}
  </>;
}
...
function BriefContent({ brief }) {
  return <div className="writing-brief-grid">
    ...
    <div className="writing-brief-block"><strong>目标受众 / 账号适配</strong><p>目标受众：{display(brief.targetAudience)}</p><p>账号定位：{display(brief.accountFit?.positioning)}</p><p>适配状态：{display(brief.accountFit?.status)}</p>{list(brief.accountFit?.reasons).map((value, index) => <small key={index}>{value}</small>)}</div>
...
function DraftEditor({ brief, title, body, busy, onTitle, onBody, onSave, onBack }) {
  ...
  return <section className="writing-draft-card">
    ...
    <div className="writing-mvp-actions"><button className="button button--secondary" onClick={onBack}>返回机会页</button><button className="button button--primary" onClick={onSave} disabled={busy !== '' || !title.trim() || !body.trim()}>{busy === 'save' ? '正在保存' : '保存草稿'}</button></div>
  </section>;
}

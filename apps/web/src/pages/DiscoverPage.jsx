...
import { evaluateOpportunity, getDiscovery, listCreators, listSignals } from '../api';
...
function SignalTable({ signals, creatorsByUser, assessmentsBySignal, onEvaluate, evaluatingSignalId }) {
  return <div className="signal-table-wrap"><table className="signal-table"><thead><tr><th>笔记</th><th>作者 / 粉丝</th><th>互动</th><th>发布时间</th><th>来源</th><th>采集时间</th><th>机会</th></tr></thead><tbody>{signals.map((signal) => {
    const creator = creatorsByUser.get(signal.author.id);
    const assessment = assessmentsBySignal.get(signal.id);
    return <tr key={signal.id}><td><strong>{value(signal.title)}</strong><small>{signal.url ? <a href={signal.url} target="_blank" rel="noreferrer">打开原链接</a> : empty}</small></td><td>{value(signal.author.name)}<small>{metric(creator?.metrics?.followers ?? signal.author.followerCount)} 粉丝</small>{!creator && <em className="evidence-gap">缺博主资料</em>}{(!assessment || assessment.baseline.sampleCount < 3) && <em className="evidence-gap">缺作者近期基线</em>}</td><td><span>赞 {metric(signal.metrics.likes)}</span><span>藏 {metric(signal.metrics.favorites)}</span><span>评 {metric(signal.metrics.comments)}</span><span>转 {metric(signal.metrics.shares)}</span></td><td>{date(signal.publishedAt)}</td><td><StatusTag tone="neutral">{role(signal)}</StatusTag><small>{value(signal.source.method)} · {observations(signal).length} 次观察</small>{observations(signal).filter((item) => item.keyword).map((item) => <small key={`${item.taskId}:${item.keyword}`}>关键词：{item.keyword}</small>)}</td><td>{date(signal.capturedAt)}</td><td><button className="card-action" disabled={Boolean(evaluatingSignalId)} onClick={() => onEvaluate(signal.id)}>{evaluatingSignalId === signal.id ? '评估中…' : '评估机会'}</button></td></tr>;
  })}</tbody></table></div>;
}
...
export function DiscoverPage({ onNavigate }) {
  ...
  const [evaluatingSignalId, setEvaluatingSignalId] = useState('');
  ...
  const evaluateSignal = async (signalId) => {
    setEvaluatingSignalId(signalId); setEvaluationError('');
    try { await evaluateOpportunity(signalId); onNavigate?.('opportunities'); }
    catch (error) { setEvaluationError(error.message || '机会评估失败'); }
    finally { setEvaluatingSignalId(''); }
  };
  ...
  return <>
    <PageHeader title="素材" description="真实 Signal 素材库；缺少基线、聚类或账号资料时也可以先评估机会。" actions={<button className="button button--primary" onClick={load}>刷新数据</button>} />
    ...
    {evaluationError && <div className="page-message page-message--error" role="alert">{evaluationError}</div>}
    <div className="hot-filter-tabs" aria-label="素材筛选">{tabs.map(([key, label]) => <button key={key} className={filter === key ? 'hot-filter-tab hot-filter-tab--active' : 'hot-filter-tab'} onClick={() => setFilter(key)}>{label}</button>)}</div>
    ...
    {state === 'ready' && ['all', 'latest'].includes(filter) && (visibleSignals.length ? <SignalTable signals={visibleSignals} creatorsByUser={creatorsByUser} assessmentsBySignal={assessmentsBySignal} onEvaluate={evaluateSignal} evaluatingSignalId={evaluatingSignalId} /> : <div className="hot-empty-state"><strong>{filter === 'latest' ? '暂无最新采集批次' : '尚未采集到真实内容'}</strong><p>系统不会使用演示数据代替。</p></div>)}

...
try { const first = await run({ REDBOOK_SMOKE_WRITE_ACCOUNT: '1' }); const second = await run({ REDBOOK_SMOKE_EXPECT_ACCOUNT: '1' }); if (!first.includes('REDBOOK_DESKTOP_SMOKE_OK') || !second.includes('REDBOOK_DESKTOP_SMOKE_OK') || !first.includes('REDBOOK_DESKTOP_RENDERER_SMOKE_OK') || !second.includes('REDBOOK_DESKTOP_RENDERER_SMOKE_OK')) throw new Error(`${first}\n${second}`); process.stdout.write(first); process.stdout.write(second); console.log('REDBOOK_DESKTOP_PERSISTENCE_OK'); } finally { await rm(runtime, { recursive: true, force: true }); }
